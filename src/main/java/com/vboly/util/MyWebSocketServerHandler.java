...
public class MyWebSocketServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    protected void messageReceived(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        // WebSocket接入
            ...
            if ("android".equals(ctx.attr(AttributeKey.valueOf("type")).get())) {
    ...
    private void handlerWebSocketFrame2(ChannelHandlerContext ctx, WebSocketFrame frame) {
        ...
        if(request.equals("PACK")){
            json.put("directive", request);
        }
        System.out.println("服务端2收到：" + request);
        TextWebSocketFrame tws = new TextWebSocketFrame(json.toString());
        // 群发
        Global.group.writeAndFlush(tws);
        // 返回【谁发的发给谁】
        // ctx.channel().writeAndFlush(tws);
